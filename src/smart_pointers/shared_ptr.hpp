...
struct control_block_with_ptr : control_block<T, Deleter> {
    ...
    control_block_with_ptr(T* p, Deleter d) noexcept : control_block<T, Deleter>(d), ptr{p} {}
    virtual ~control_block_with_ptr() = default;
    auto delete_obj() -> void override { this->deleter(ptr); }
};

template <class T>
struct control_block_with_obj : control_block<T> {
...
class shared_ptr {
  public:
    ...
    constexpr shared_ptr(std::nullptr_t) noexcept : _ptr{nullptr}, _cb_ptr{nullptr} {}

    // regular constructors
    template <detail::pointer_convertible_to<T> U>
    explicit shared_ptr(U* ptr) : _ptr{ptr}, _cb_ptr{new detail::control_block_with_ptr<U>(ptr)} {}

    template <detail::pointer_convertible_to<T> U, std::invocable<U*> Deleter>
    shared_ptr(U* ptr, Deleter d) : _ptr{ptr}, _cb_ptr{new detail::control_block_with_ptr<U>(ptr, d)} {}

    // copy/move constructors
    shared_ptr(const shared_ptr& other) noexcept : _ptr{other._ptr}, _cb_ptr{other._cb_ptr} {
        if (_ptr != nullptr) 
            _cb_ptr->shared_count.fetch_add(1, std::memory_order_relaxed);
    }
    shared_ptr(shared_ptr&& other) noexcept
        : _ptr{std::exchange(other._ptr, nullptr)},
          _cb_ptr{std::exchange(other._cb_ptr, nullptr)} {}

    template <detail::pointer_convertible_to<T> U>
    shared_ptr(const shared_ptr<U>& other) noexcept : _ptr{other._ptr}, _cb_ptr{other._cb_ptr} {
        if (_ptr != nullptr) _cb_ptr->shared_count.fetch_add(1, std::memory_order_relaxed);
    }
    ...
    shared_ptr(unique_ptr<U, Deleter>&& r) {
        if (r) {
            if constexpr (std::is_reference_v<Deleter>) {
                _cb_ptr = ::new detail::control_block_with_ptr<U>(r.get(), r.get_deleter());
            }
            else {
                _cb_ptr = ::new detail::control_block_with_ptr<U>(r.get(), std::move(r.get_deleter()));
            }
            _ptr = r.release();
        }
        else {
    ...
    template <class U>
    shared_ptr(const shared_ptr<U>& r, element_type* ptr) noexcept : _ptr{ptr}, _cb_ptr{r._cb_ptr} {
        if (r) _cb_ptr->shared_count.fetch_add(1, std::memory_order_relaxed);
    }

    template <class U>
    shared_ptr(shared_ptr<U>&& r, element_type* ptr) noexcept : _ptr{ptr}, _cb_ptr{std::exchange(r._cb_ptr, nullptr)} {
        r._ptr = nullptr;
    }
    ...
    auto operator*() const
        noexcept(noexcept(*std::declval<element_type*>())) -> element_type& {
        return *get();
    }
    auto operator->() const noexcept -> element_type* { return get(); }

    auto use_count() const noexcept -> long { return _cb_ptr ? _cb_ptr->shared_count.load(std::memory_order_relaxed) : 0; }
    explicit operator bool() const noexcept { return get() != nullptr; }

    // comparison
    template <class U>
    [[nodiscard]] auto operator==(const shared_ptr<U>& rhs) const noexcept -> bool { return get() == rhs.get(); }

    [[nodiscard]] auto operator==(std::nullptr_t) const noexcept -> bool { return get() == nullptr; }
